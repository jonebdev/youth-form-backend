...
module.exports = async (req, res) => {
  ...
  const parent_name = createCustomField(field_id.parent_name, values.person.parent_name);
  const email = createEmail(values.person.email);
  const cell = createCell(values.person.cell);
  const school = createCustomField(field_id.children_school, values.person.children_school);
  const health = createCustomField(field_id.children_health, values.person.children_health);
  const howYouHeard = createCustomField(field_id.children_how_you_heard, values.person.children_how_you_heard);
